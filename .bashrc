...
# Función para mostrar la rama de git
parse_git_branch() {
    git branch 2> /dev/null | sed -e '/^[^*]/d' -e 's/* \(.*\)/ (\1)/'
}

parse_node_version() {
    command -v node >/dev/null 2>&1 && echo " ⬢ $(node -v)"
}
...
# Created by `pipx` on 2026-06-24 18:52:36
export PATH="$PATH:/home/wsldev/.local/bin"
